...
module.exports.run = async(client, message, args) => {
        ...
        let timeout = 3600000;
        let author = await db.fetch(`workedhour_${message.guild.id}_${user.id}`);
        ...
        if(author !== null && timeout - (Date.now() - author) > 0){
            ...
            db.set(`workedhour_${message.guild.id}_${user.id}`, Date.now())

            message.channel.send({
                embed: {
                    color: 0x666edf,
                    title: "<:Poussire:867067966792269824> Extraction **modérée** !",
                    author:{
